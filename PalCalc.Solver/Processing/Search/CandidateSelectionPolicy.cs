...
internal sealed class DefaultCandidateSelectionPolicy : ICandidateSelectionPolicy
{
    ...
    )
    {
        ...
        comparison = candidate.IVs.RelevantMaxTotal.CompareTo(
            incumbent.IVs.RelevantMaxTotal
        );
        if (comparison != 0)
            return comparison > 0
                ? FrontierCandidateAssessment.PotentialImprovement
        ...
        comparison = candidate.IVs.RelevantMinTotal.CompareTo(
            incumbent.IVs.RelevantMinTotal
        );
        if (comparison != 0)
            return comparison > 0
                ? FrontierCandidateAssessment.PotentialImprovement
